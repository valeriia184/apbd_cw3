...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldReturnAllSalesmen()
    {
        ...
        List<Emp> result = emps.Where(s => s.Job == "SALESMAN").ToList<Emp>();
    ...
    public void ShouldReturnDept30EmpsOrderedBySalaryDesc()
    {
        ...
        List<Emp> result = emps.Where(s => s.DeptNo == 30).OrderByDescending(s => s.Sal).ToList<Emp>();  
    ...
    public void ShouldReturnEmployeesFromChicago()
    {
        ...
        List<Emp> result = emps.Where(emp => depts 
            ...
            .Contains(emp.DeptNo)).ToList<Emp>();
    ...
    public void ShouldSelectNamesAndSalaries()
    {
        ...
        var result = emps.Select(s => new {s.EName, s.Sal}).ToList();
    ...
    public void ShouldCountEmployeesPerDepartment()
    {
        var emps = Database.GetEmps();
         ...
         var result = emps.GroupBy(emp => emp.DeptNo).Select(group => new {DeptNo = group.Key, Count = group.Count()}).ToList(); 
    ...
    public void ShouldMatchEmployeeToSalaryGrade()
    {
        ...
        var grades = Database.GetSalgrades();
         ...
         var result = emps.SelectMany(emp => grades, (emp, grade) => new {emp, grade})
             .Where(e => e.emp.Sal >= e.grade.Losal && e.emp.Sal <= e.grade.Hisal)
             .Select(s => new {s.emp.EName, s.grade.Grade}).ToList();
    ...
    public void ShouldCalculateAverageSalaryPerDept()
    {
        ...
        var result = emps.GroupBy(emp => emp.DeptNo).Select(group => new {DeptNo = group.Key, AvgSal = group.Average(emp => emp.Sal)}).ToList(); 
